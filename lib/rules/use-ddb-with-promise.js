...
module.exports = {
  ...
  create: function(context) {
    ...
    function findDDBReferencesFromDDBRequireDestructs(context) {
      let references = [];
      ddbRequireDestructs.forEach(n => {
        context
          .getDeclaredVariables(n)
          .filter(c => {
            const { identifiers } = c;
            return (
              identifiers[0].parent.key &&
              identifiers[0].parent.key.name == "DynamoDB"
            );
          })
          .forEach(node => {
            console.log("customNode>", node);
            references = references.concat(node.references);
          });
      });
      return references;
    }
    return {
      "Program:exit"(node) {
        ...
        references1.concat(references2).forEach(node => {
