  ...
  create: function(context) {
    ...
    return {
      "Program:exit"(node) {
        ...
        const references1 = findDDBReferencesFromRequireDestructs(
          ddbRequireDestructs
        );
        const references2 = findDDBReferencesFromNodes(nodes);
        const references3 = findDDBReferencesFromDocumentClientDestructs(
          documentClientDestructs
        );
        const references = references1.concat(references2).concat(references3);
      ...
      VariableDeclaration(node) {
        //check if node is require(aws-sdk)
        //OR
        //created from the variable used with
        //require(aws-sdk)/import aws-sdk
        ...
        const ddbRequireNames = ddbRequireDestructs
          ...
          .filter(Boolean);
        const awsVariableNames = awsImportNames
          .concat(awsRequireNames)
          .concat(ddbRequireNames);
        const declarations = node.declarations.filter(declaration => {
          return declaration.init.arguments
            ? declaration.init.arguments.find(
                ({ value }) => value === "aws-sdk"
              )
            : awsVariableNames.includes(declaration.init.name);
        });
    ...
    //#region Utils
    function findDDBVariableDeclaratorsFromNodes(nodes = []) {
    ...
    function findDDBVariableDeclaratorsFromRequireDestructs(nodes = []) {
    ...
    function findDDBVariableDeclaratorsFromDocumentClientDestructs(nodes = []) {
      let variableDeclarators = [];
      nodes.forEach(node => {
        const vars = context.getDeclaredVariables(node);
        console.log(
          "findDDBVariableDeclaratorsFromRequireDestructs: node>",
          node
        );
        console.log(
          "findDDBVariableDeclaratorsFromRequireDestructs: vars>",
          vars
        );
        const refs = vars[0].references.slice(1);
        console.log(
          "findDDBVariableDeclaratorsFromRequireDestructs: refs>",
          refs
        );
        refs.forEach(ref => {
          const variableDeclarator = findParent(ref.identifier, {
    ...
    function findDDBReferencesFromDocumentClientDestructs(nodes = []) {
      const variableDeclarators = findDDBVariableDeclaratorsFromDocumentClientDestructs(
        nodes
      );
      return findDDBReferencesFromVariableDeclarators(variableDeclarators);
    }
    function findDDBReferencesFromVariableDeclarators(nodes = []) {
      let ddbReferences = [];
      nodes.forEach(node => {
    ...
    function findDDBReferencesFromRequireDestructs(nodes = []) {
      const variableDeclarators = findDDBVariableDeclaratorsFromRequireDestructs(
        nodes
      );
      return findDDBReferencesFromVariableDeclarators(variableDeclarators);
    }
    function findDDBReferencesFromNodes(nodes = []) {
      const variableDeclarators = findDDBVariableDeclaratorsFromNodes(nodes);
      return findDDBReferencesFromVariableDeclarators(variableDeclarators);
    }
