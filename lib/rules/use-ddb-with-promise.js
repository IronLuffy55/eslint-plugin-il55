...
module.exports = {
  ...
  create: function(context) {
    ...
    function findDDBVariableDeclaratorsFromNode(context) {
      const variableDeclarators = [];
      awsImports
        .concat(awsRequires)
        .concat(ddbImportDestructs)
        .forEach(node => {
    ...
    function findDDBReferencesFromVariableDeclarators(nodes = [], context) {
      let ddbReferences = [];
      nodes.forEach(node => {
    ...
    function findDDBVariableDeclaratorsFromRequireDestructs(context) {
      let variableDeclarators = [];
      ddbRequireDestructs.forEach(node => {
      ...
      console.log("variableDeclarators>", variableDeclarators);
      return variableDeclarators;
    }
    function findDDBReferencesFromDDBRequireDestructs(context) {
      const variableDeclarators = findDDBVariableDeclaratorsFromRequireDestructs(
        context
      );
      return findDDBReferencesFromVariableDeclarators(
        variableDeclarators,
        context
      );
    }
    return {
      "Program:exit"(node) {
        const references1 = findDDBReferencesFromDDBRequireDestructs(context);
        const ddbVariableDeclarators = findDDBVariableDeclaratorsFromNode(
          context
        );
        const references2 = findDDBReferencesFromVariableDeclarators(
          ddbVariableDeclarators,
          context
        );
        const ddbReferences = references1.concat(references2);
      ...
      VariableDeclaration(node) {
        ...
        node.declarations.forEach(variableDeclaratornNode => {
          if (variableDeclaratornNode.id.type == "ObjectPattern") {
            //deconstructed
            const ddbPropertyNode = variableDeclaratornNode.id.properties.find(
              ({ key }) => key.name == "DynamoDB"
            );
            if (!ddbPropertyNode) return;
            ddbRequireDestructs.push(variableDeclaratornNode);
